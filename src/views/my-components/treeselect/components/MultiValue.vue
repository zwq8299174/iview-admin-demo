  ...
  <div class="vue-treeselect__value-wrapper">
    <transition-group
      ...
      <template v-for="node in instance.visibleValue">
        <div class="vue-treeselect__multi-value-item-wrapper" @mousedown="handleMouseDownOnValue" :key="node.id">
          <div :class="[ 'vue-treeselect__multi-value-item', { 'vue-treeselect__multi-value-item-disabled': node.isDisabled } ]">
            <span class="vue-treeselect__multi-value-label">{{ node[[instance.label]] }}</span><span class="vue-treeselect__icon vue-treeselect__value-remove" @mousedown="instance.select(node)">&times;</span>
          </div>
        </div>
      </template>
